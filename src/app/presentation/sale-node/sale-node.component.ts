...
export class SaleNodeComponent implements OnInit, OnDestroy, OnChanges {
  ...
  get hasPromo(): boolean {
    ...
    if (promo === null || price === null) {
      return false;
    }
    if (!(promo > 0 && price > 0 && promo < price)) {
      return false;
    }
    return !this.promoExpired;
  }
  ...
  private resolveColumn(field: 'leftImage' | 'rightImage' | 'leftText' | 'rightText'): string {
    ...
    if (typeof flat === 'string' && flat.trim().length) {
      return flat;
    }
    const nestedVal = this.nested[field];
    if (typeof nestedVal === 'string' && nestedVal.trim().length) {
      return nestedVal;
    }
    return '';
  }
